...
if (matched == "Yes") {
  ...
  df_base <-
    ...
    left_join(
      readRDS(paste0(working_path, "dNPV_out_nonmissing_dnpv.rds")),
      by = "fncl_ast_id"
    )
} else if (matched == "No") {
  df_base <- readRDS(paste0(working_path,
                            "matched_hamp_yearblock_unc_sub_um.rds")) %>%
    select(fncl_ast_id, mod_mnth_hamp)
}
rm(matched_hamp)
...
mrgdHamp <-
  ...
  filter(abs(x) < 6 & ivsr_grpNum == "Non-GSE")
...
mrgdHamp_toadd %>% filter(abs(x) < 6) %>%
  saveRDS(file = paste0(working_path, "rd_samp_unmatched_long", suffix, ".rds"))

mrgdHamp_toadd %>%
  filter(praSampCore == TRUE & abs(x) < 4 & mdfcCohort >= 1331) %>%
  saveRDS(file = paste0(working_path, "rd_samp_unmatched", suffix, ".rds"))
